...
class MatchIf {
 public:
  ...
  virtual int32_t rempve_user(const User& user, const std::string& info) = 0;
};
...
class MatchNull : virtual public MatchIf {
 public:
  ...
  int32_t rempve_user(const User& /* user */, const std::string& /* info */) override {
    int32_t _return = 0;
    return _return;
  }
};
...
typedef struct _Match_rempve_user_args__isset {
  _Match_rempve_user_args__isset() : user(false), info(false) {}
  bool user :1;
  bool info :1;
} _Match_rempve_user_args__isset;

class Match_rempve_user_args {
 public:

  Match_rempve_user_args(const Match_rempve_user_args&);
  Match_rempve_user_args& operator=(const Match_rempve_user_args&);
  Match_rempve_user_args() noexcept
                         : info() {
  }

  virtual ~Match_rempve_user_args() noexcept;
  User user;
  std::string info;

  _Match_rempve_user_args__isset __isset;

  void __set_user(const User& val);

  void __set_info(const std::string& val);

  bool operator == (const Match_rempve_user_args & rhs) const
  {
    if (!(user == rhs.user))
      return false;
    if (!(info == rhs.info))
  ...
  bool operator < (const Match_rempve_user_args & ) const;
...
class Match_rempve_user_pargs {
 public:


  virtual ~Match_rempve_user_pargs() noexcept;
  const User* user;
  const std::string* info;
...
typedef struct _Match_rempve_user_result__isset {
  _Match_rempve_user_result__isset() : success(false) {}
  bool success :1;
} _Match_rempve_user_result__isset;

class Match_rempve_user_result {
 public:

  Match_rempve_user_result(const Match_rempve_user_result&) noexcept;
  Match_rempve_user_result& operator=(const Match_rempve_user_result&) noexcept;
  Match_rempve_user_result() noexcept
                           : success(0) {
  }

  virtual ~Match_rempve_user_result() noexcept;
  int32_t success;

  _Match_rempve_user_result__isset __isset;

  void __set_success(const int32_t val);

  bool operator == (const Match_rempve_user_result & rhs) const
  {
    if (!(success == rhs.success))
      return false;
    return true;
  ...
  bool operator < (const Match_rempve_user_result & ) const;
...
typedef struct _Match_rempve_user_presult__isset {
  _Match_rempve_user_presult__isset() : success(false) {}
  bool success :1;
} _Match_rempve_user_presult__isset;

class Match_rempve_user_presult {
 public:


  virtual ~Match_rempve_user_presult() noexcept;
  int32_t* success;

  _Match_rempve_user_presult__isset __isset;
...
class MatchClient : virtual public MatchIf {
 ...
 public:
  ...
  int32_t rempve_user(const User& user, const std::string& info) override;
  void send_rempve_user(const User& user, const std::string& info);
  int32_t recv_rempve_user();
 protected:
...
class MatchProcessor : public ::apache::thrift::TDispatchProcessor {
 ...
 private:
  ...
  void process_rempve_user(int32_t seqid, ::apache::thrift::protocol::TProtocol* iprot, ::apache::thrift::protocol::TProtocol* oprot, void* callContext);
 public:
  MatchProcessor(::std::shared_ptr<MatchIf> iface) :
    iface_(iface) {
    processMap_["add_user"] = &MatchProcessor::process_add_user;
    processMap_["rempve_user"] = &MatchProcessor::process_rempve_user;
  }
...
class MatchMultiface : virtual public MatchIf {
 ...
 public:
  ...
  int32_t rempve_user(const User& user, const std::string& info) override {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->rempve_user(user, info);
    }
    return ifaces_[i]->rempve_user(user, info);
  }
  ...
  MatchConcurrentClient(std::shared_ptr< ::apache::thrift::protocol::TProtocol> iprot, std::shared_ptr< ::apache::thrift::protocol::TProtocol> oprot, std::shared_ptr<::apache::thrift::async::TConcurrentClientSyncInfo> sync) : sync_(sync)
{
 ...
 public:
  ...
  int32_t rempve_user(const User& user, const std::string& info) override;
  int32_t send_rempve_user(const User& user, const std::string& info);
  int32_t recv_rempve_user(const int32_t seqid);
 protected:
  std::shared_ptr< ::apache::thrift::protocol::TProtocol> piprot_;
  std::shared_ptr< ::apache::thrift::protocol::TProtocol> poprot_;
  ::apache::thrift::protocol::TProtocol* iprot_;
